...
class SingleLegSideTensor():
    """
    The class represents a tensor that has only a single leg in one of his sides.
    The single leg will always be index 0
    """

    def __init__(self, num_qudits: int, radixes: Sequence[int] = [], list_of_states:Sequence=[], tensor=None, single_leg_radix=None) -> None:
        ...
        if len(list_of_states) > 0:
            ...
            self.tensor = jnp.array(list_of_states).reshape(self.single_leg_radix, *self.radixes)
        elif tensor != None:
            self.tensor = tensor
            ...
            raise "can't create the instance"

    def copy(self)->SingleLegSideTensor:
        return self.__class__(tensor=self.tensor.copy(), num_qudits = self.num_qudits,radixes =  self.radixes, single_leg_radix = self.single_leg_radix)

    @staticmethod
    def calc_env(left:LHSTensor, right:RHSTensor, indexes_to_leave_open:Sequence[int]):
        ...
        for leg_num, i in enumerate(indexes_to_leave_open):
            left_contraction_indexs[i+1] = size_of_open+leg_num+left.num_of_legs
...
class RHSTensor(SingleLegSideTensor):
    ...
    def apply_left(
        ...
        utry_tensor_indexs = [i+self.num_of_legs for i in range(utry_size)] + [1+l for l in location]
...
class LHSTensor(SingleLegSideTensor):
    ...
    def apply_right(
        ...
        utry_tensor_indexs = [1+l for l in location] + [i+self.num_of_legs for i in range(utry_size)] 
